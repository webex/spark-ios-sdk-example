...
class CreateGroupViewControlller: BaseViewController ,UITextFieldDelegate ,UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource{
    ...
    // MARK: - SparkSDK: Create Spark Space
    public func createNewSpace(){
        ...
        self.sparkSDK?.rooms.create(title: spaceTitle) { (response: ServiceResponse<Room>) in
            switch response.result {
            case .success(let value):
                let threahGroup = DispatchGroup()
                for person in self.selectedPersonList{
                    DispatchQueue.global().async(group: threahGroup, execute: {
                        self.sparkSDK?.memberships.create(roomId: value.id!, personEmail:(person.emails?.first)!, completionHandler: { (response: ServiceResponse<Membership>) in
                            switch response.result{
                            case .success(_):
                                break
                            case .failure(_):
